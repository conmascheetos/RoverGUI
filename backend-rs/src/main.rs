...
async fn get_camera_feed(camera_path: &str, offer: Json<RTCSessionDescription>, state: &State<AppState>) -> Result<Json<RTCSessionDescription>, Status> {
    let webcam_manager = &state.webcam_manager;
    let local_description = webcam_manager.add_client(camera_path.to_owned(), offer.into_inner()).await
...
async fn get_camera_mode(camera_path: &str, state: &State<AppState>) -> Result<String, (Status, &'static str)> {
    let camera_handles_mutex = state.webcam_manager.camera_handles();
    let camera_handles = &mut *camera_handles_mutex.lock().await;
    let handle = camera_handles.iter().find(|handle| handle.camera_path() == camera_path)
...
async fn get_camera_modes(camera_path: &str, state: &State<AppState>) -> Result<Json<HashMap<usize, String>>, (Status, &'static str)> {
    let camera_handles_mutex = state.webcam_manager.camera_handles();
    let camera_handles = &mut *camera_handles_mutex.lock().await;
    let handle = camera_handles.iter().find(|handle| handle.camera_path() == camera_path)
        .ok_or((Status::BadRequest, "Invalid / Inactive Camera Path"))?;
...
async fn get_camera_mode_set(camera_path: &str, mode_id: usize, state: &State<AppState>) -> Result<(), (Status, &'static str)> {
    let camera_handles_mutex = state.webcam_manager.camera_handles();
    let camera_handles = &mut *camera_handles_mutex.lock().await;
    let handle = camera_handles.iter_mut().find(|handle| handle.camera_path() == camera_path)
        .ok_or((Status::BadRequest, "Invalid / Inactive Camera Path"))?;
    handle.update_camera_mode(mode_id).await.map_err(|_| (Status::BadRequest, "Failed to Update Camera Mode; index may be invalid"))?;
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    for node in v4l::context::enum_devices() {
        ...
        let initial_mode = modes.first().ok_or("No Available Modes")?.clone();
    ...
    rocket::build()
        ...
        .mount("/stream", routes![get_available_cameras, get_camera_feed, get_camera_mode, get_camera_modes, get_camera_mode_set])
        .manage(AppState {
